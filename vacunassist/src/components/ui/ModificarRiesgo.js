      ...
      <Sidebar33 />
        <div className="">
          <div className="text-white font-bold text-4xl flex-start px-5 py-2 flex">
            Modificar estado de riesgo de un paciente
          </div>
        ...
        paciente.map((paciente, index) => (
          <div key={index}>
            <span className="text-white font-bold flex-start flex pl-2 py-3">
              Nombre: {paciente.name} | Apellido: {paciente.lastname} | DNI: {paciente.dni} | Email: {paciente.email} | Riesgo: {paciente.riesgo ? "Sí" : "No"}
              <button className="text-white rounded font-bold boton-activo"
              onClick={() => {

                axios.post('http://localhost:4000/api/user/modificarriesgo', paciente).then(
                    Swal.fire('', 'Modificaste el estado de riesgo del paciente con éxito', 'success').then(function() {
...
export default ModificarRiesgo;
